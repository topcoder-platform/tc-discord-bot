...
export class MembersCheckJob implements Job {
    ...
    public async run(): Promise<void> {
                    ...
                    if (dbM) {
                        ...
                        await this.shardManager.broadcastEval(
                            async (client, context) => {
                                ...
                                if (
                                    context.tcHandle !== (member.nickname || member.user.username)
                                ) {
                                ...
                                if (!member.roles.cache.has(context.ratingRole)) {
                                    await member.roles.remove([
                                        context.grayRatedRoleID,
                                        context.greenRatedRoleID,
                                        context.blueRatedRoleID,
                                        context.yellowRatedRoleID,
                                        context.redRatedRoleID,
                                        context.targetRatedRoleID,
                                    ]);
                        ...
                        await this.shardManager.broadcastEval(
                            async (client, context) => {
                                ...
                                if (
                                    ![context.roleCM, context.roleADMIN].some(r =>
        ...
        Logger.info('Ran Members Check job and finished okay');
    }
}
